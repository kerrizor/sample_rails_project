...
describe ClearancingService do
  ...
  describe "::process_array" do
    context "total success" do
      let(:items) { 5.times.map{ FactoryGirl.create(:item) }.collect(&:id) }

      before do
        @clearancing_status = clearancing_service.process_array(items)
      end
      ...
      it "adds all the items to the batch" do
        expect(@clearancing_status.clearance_batch.items.pluck(:id).sort).to eq(items.sort)
      end
    ...
    context "partial success" do
      ...
      before do
        @clearancing_status = clearancing_service.process_array(valid_items + invalid_items)
      end
    ...
    context "total failure" do
      ...
      before do
        @clearancing_status = clearancing_service.process_array(invalid_items)
      end
